...
- **NameSearch facets** (item 1's metadata axis): query by path/extension/`media_kind` ("pdfs in the active channel", "images touched this week").
...
5. **Uploads ↔ artifacts unification — keying.** **UPDATE 2026-06-25: RESOLVED — uploads now AUTO-LAND as `shared/channels/<channel-id>/uploads/` artifacts by default** (reverses the "attachments stay in `files`, separate metadata" lean below). The `files` table becomes the ingest record; the upload creates a shared artifact pointing at the same CAS blob (zero-copy via `content_hash`). Chat immutability is preserved by version-pinning the message's `(artifact, seq)`, not by separate storage. Canonical: `notes/shared-workspace-build-spec.md` §0.2. (Earlier keying analysis kept below for lineage.) *(Refined 2026-06-22 after a code-verified trace of both paths.)* **Verified facts:** same bucket; disjoint key prefixes; separate tables with no FK/link; no crossover flow; uploads are id-keyed with opportunistic *per-uploader* dedup via an **already-present `content_hash` column** (`020_upload_content_hash.sql`), artifacts are sha-keyed/globally-dedup'd. The **experience unification is the valuable, non-negotiable part** (both `files` and `artifacts` flow through one `MediaObject` seam → same `media_kind`/extract/preview/`atrium cat`), and it's **separable from byte keying**. **The metadata schemas should stay separate** — they model different identity/lifecycle/auth (message-scoped immutable attachment vs. versioned conflict-aware path); merging them buys nothing and imports complexity. The byte-keying spectrum:
...
## 10. PREREQUISITE (verified 2026-06-22, BOTH repos): shared artifact workspace is the committed design but is **not built**

The product intent (Gary) is a shared artifact workspace where humans can drop files and agents read **and edit** them like any artifact. The 2026-06-25 mount clarification makes the agent-facing default one active shared leaf at `~`, with explicit session scratch at `~/scratch` and wider granted scopes under `~/shared`. But a code-verified trace of **both Atrium and Centaur** shows it is **not implemented on either side** today; each agent's current `/workspace` is strictly its own session's files. This is "build this," not "wrong direction" — but it's a sizeable cross-repo build, not a small on-ramp.
