...
import { sessionsApi, type PublishedArtifactApp } from './api';
...
function detectAppRoots(artifacts: Artifact[]): DetectedAppRoot[] {
  ...
  for (const artifact of artifacts) {
    ...
    if (!roots.has(rootPath)) roots.set(rootPath, { name: name!.toLowerCase(), rootPath, entry: 'index.html' });
  }
...
export function AppsSurface({
  ...
  const [apps, setApps] = useState<PublishedArtifactApp[]>([]);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const detected = useMemo(() => detectAppRoots(artifacts), [artifacts]);
  const publishedRoots = useMemo(() => new Set(apps.map((app) => app.rootPath)), [apps]);
  const unpublishedDetected = detected.filter((root) => !publishedRoots.has(root.rootPath));

  async function refresh() {
    setLoading(true);
    setError(null);
    try {
      const response = await sessionsApi.listApps(sessionId);
  ...
  async function publish(root: DetectedAppRoot) {
    setBusy(`publish:${root.rootPath}`);
    setError(null);
    try {
      await sessionsApi.publishApp(sessionId, root);
      await refresh();
    } catch (err) {
  ...
  async function launch(app: PublishedArtifactApp) {
    setBusy(`launch:${app.appId}`);
    setError(null);
    try {
      const { url } = await sessionsApi.launchApp(sessionId, app.appId);
          ...
          <div className="divide-y divide-edge">
            {unpublishedDetected.map((root) => (
              <div key={root.rootPath} className="flex items-center gap-3 px-3 py-2">
                ...
                >
                  {busy === `publish:${root.rootPath}` ? 'Publishing' : 'Publish'}
                </button>
              </div>
            ))}
          </div>
      ...
      {loading ? (
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-2 p-3 sm:grid-cols-2">
            {apps.map((app) => (
              <article key={app.appId} className="rounded-md border border-edge bg-surface-raised/50 p-3">
                <div className="flex items-start gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 truncate font-mono text-3xs text-fg-muted">
                      {app.rootPath}{app.entry}
                    </div>
                    {app.description && (
                      <div className="mt-1 line-clamp-2 text-xs text-fg-secondary">{app.description}</div>
                    )}
                  </div>
                ...
                <div className="mt-3 flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => launch(app)}
                    disabled={busy != null || app.status !== 'published'}
                    className="rounded border border-accent-border px-2 py-1 text-3xs font-semibold uppercase tracking-wide text-accent-text hover:bg-accent-soft disabled:cursor-not-allowed disabled:opacity-60"
                  >
                    {busy === `launch:${app.appId}` ? 'Launching' : 'Launch'}
                  </button>
  ...
  return (
    <div data-testid="apps-surface" className="absolute inset-0 z-10 flex flex-col bg-surface/95 backdrop-blur-sm">
      {body}
    </div>
  );
}
