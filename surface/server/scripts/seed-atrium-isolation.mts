...
const pool = new pg.Pool({ connectionString: process.env.DATABASE_URL ?? 'postgres://atrium:atrium@localhost:5433/atrium' });
const ws = (await pool.query<{ id: string }>(`SELECT id FROM workspaces ORDER BY created_at ASC LIMIT 1`)).rows[0]!.id;
const pubCh = (await pool.query<{ id: string }>(
  `SELECT id FROM channels WHERE workspace_id=$1 AND kind='public' ORDER BY created_at ASC LIMIT 1`, [ws],
)).rows[0]!.id;
...
async function user(handle: string): Promise<string> {
  ...
  await pool.query(`INSERT INTO workspace_members (workspace_id, user_id) VALUES ($1,$2) ON CONFLICT DO NOTHING`, [ws, r.rows[0]!.id]);
  return r.rows[0]!.id;
}
...
await pool.query(`INSERT INTO channel_members (channel_id, user_id) VALUES ($1,$2) ON CONFLICT DO NOTHING`, [priv, alice]);
...
for (const data of frames)
  await pool.query(`INSERT INTO session_events (session_id, centaur_event_id, event_kind, frame) VALUES ($1,$2,'amp_raw_event',$3) ON CONFLICT DO NOTHING`,
