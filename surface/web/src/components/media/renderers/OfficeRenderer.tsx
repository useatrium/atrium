...
import { fileExtension, formatBytes, isDocxFile, officeFileKind } from '../utils';
...
export function OfficeRenderer({ file, variant }: { file: PreviewFile; variant: MediaPreviewVariant }) {
  ...
  const unsupportedMessage = useMemo(() => {
    if (officeKind === 'presentation') {
      return 'PowerPoint preview is not available in the web renderer yet. Download the file to view slides.';
    }
