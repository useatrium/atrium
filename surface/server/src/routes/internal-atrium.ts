...
export function registerInternalAtriumRoutes(app: FastifyInstance, deps: InternalAtriumRouteDeps): void {
  ...
  app.get('/api/internal/sessions/:viewerId/atrium/channels', async (req, reply) => {
    ...
    // Project explicitly: node-sync's parser maps `lastEventId` and `last_event_id`
    // to the same field, and sending both poisons the whole channel materializer
    // (serde rejects alias duplicates). Exactly one spelling may ever be emitted.
    return reply.send(
