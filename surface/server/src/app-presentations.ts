...
export async function refreshAppPresentations(
  ...
  for (const [slug, appFiles] of dirs) {
    ...
    const last = await pool.query<{
      ...
      `SELECT version, entry_path, entry_blob_sha, manifest_blob_sha
    ...
    await pool.query(
      ...
      [
        ...
        manifest.entryPath,
        manifest.previewUrl,
        JSON.stringify(manifest.previewSizePolicy),
...
export async function listLatestAppPresentations(
  pool: Db,
  sessionId: string,
): Promise<AppPresentationRow[]> {
...
async function parseManifest(
  ...
  const entryRaw =
    stringField(raw, 'entrypoint') ??
    stringField(raw, 'entry') ??
    'index.html';
  ...
  const previewUrl = previewEnabled
    ...
    : null;
  const sizes = Array.isArray(preview?.sizes) ? preview.sizes : [];
