...
describe('fileChangeFromToolCall', () => {
  ...
  it('does not emit a ghost line for a trailing newline in old/new strings', () => {
    const fc = fileChangeFromToolCall(tool('Edit', { file_path: 'a.ts', old_string: 'foo\n', new_string: 'bar\n' }));
...
describe('collectFileChanges over a reduced stream (real frame path)', () => {
  ...
  it('merges codex + claude edits in collectFileChanges', () => {
          ...
          content: [
            {
              ...
              id: 't1',
              name: 'Edit',
              input: { file_path: 'claude.ts', old_string: 'a', new_string: 'b' },
            },
          ],
        },
      },
    ...
    state = reduceSession(state, {
      ...
      data: {
        type: 'item.completed',
        item: { id: 'fc-1', type: 'fileChange', changes: [{ path: 'codex.ts', kind: 'add', diff: '+x' }] },
      },
    } as unknown as CentaurEventFrame);
...
describe('codexInlineFileChanges (inline transcript placement)', () => {
  it('anchors each codex edit after the items that preceded it (by event id)', () => {
    ...
    state.fileChanges = [
      {
        id: 'fc-1',
        path: '/home/agent/workspace/a.ts',
        kind: 'update',
        diff: '@@\n-x\n+y',
        toolName: 'fileChange',
        sourceEventIds: [3],
      ...
      {
        id: 'fc-2',
        path: '/home/agent/workspace/b.ts',
        kind: 'delete',
        diff: '@@\n-gone',
        toolName: 'fileChange',
        sourceEventIds: [7],
