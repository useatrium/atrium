export type MediaKind = 'text' | 'image' | 'audio' | 'video' | 'pdf' | 'archive' | 'json' | 'document' | 'binary';
...
export function classifyMedia(
  ...
  const isText =
    magicMime == null &&
    textProbe.valid &&
    (mimeLooksText || extensionLooksText || declaredMime === 'application/octet-stream');
  const detectedMime =
    magicMime ?? (isText && declaredMime === 'application/octet-stream' ? 'text/plain' : declaredMime);
