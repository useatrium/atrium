...
type ActivityKind = 'mention' | 'dm' | 'agent_question' | 'session_completed';
    ...
    const res = await pool.query<ActivityRow>(
      `WITH activity AS (
         SELECT e.id AS event_id,
                ...
                e.channel_id,
                e.actor_id,
                left(coalesce(nullif(e.payload->>'text', ''), '(attachment)'), 140) AS snippet,
                e.created_at
         FROM mentions mn
         ...
         SELECT e.id AS event_id,
                'session_completed'::text AS kind,
                e.channel_id,
                e.actor_id,
                left(coalesce(
